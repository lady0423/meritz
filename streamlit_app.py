...
# 필터 함수
# ============================================================
def apply_manager_filter(agents_df, filter_mode, current_week, df_main):
    """
    filter_mode:
      0 = 전체
      1 = 현재주차 유실적자 (현재주차 컬럼 > 0)
      2 = 브릿지 유실적자 (브릿지부족최종 < 브릿지 도전구간, 즉 브릿지 달성 중)
      3 = MC 해당자 (MC도전구간 또는 MC+구간 있고 미달성 아님)
    """
    if filter_mode == 0:
        return agents_df

    if filter_mode == 1:
        week_col = ["1주차","2주차","3주차","4주차","5주차"]
        if current_week >= 1 and current_week <= 5:
            col = week_col[current_week - 1]
            if col in agents_df.columns:
                return agents_df[agents_df[col].apply(safe_float) > 0]
        return agents_df

    if filter_mode == 2:
        # 어센틱 아닌 사람 중 브릿지 누계실적 > 0 (브릿지부족최종이 숫자이고 > 0 = 아직 도전중 포함)
        def is_bridge_active(row):
            is_auth = safe_float(row.get("어센틱구분", 0)) == 1
            if is_auth:
                return False
            bridge_val = safe_float(row.get("브릿지 도전구간", 0))
            return bridge_val > 0
        mask = agents_df.apply(is_bridge_active, axis=1)
        return agents_df[mask]

    if filter_mode == 3:
        # MC 해당자: MC도전구간이 숫자이고 > 0, 또는 MC+구간 > 0
        ...
        mask = agents_df.apply(is_mc_target, axis=1)
        return agents_df[mask]
...
with tab1:
    ...
    with col1:
        ...
        selected_branch = st.selectbox("지점명", ga4_branches, index=1, label_visibility="collapsed", key="branch")
    with col2:
    ...
    if st.session_state.search_performed and st.session_state.selected_row is not None:
        ...
        with col_left:
            ...
            st.markdown(f"<div class='cumulative-box'>{format_display(cumulative)}</div>", unsafe_allow_html=True)
            ...
            if is_authentic:
                ...
                render_mc_box(row.get("MC도전구간",0), row.get("MC부족최종",0), is_authentic=True, is_mc_plus=False, mc_target_value=mc_target_value)
            else:
            ...
            with col_copy2:
                st.download_button(label="💾 텍스트 파일로 저장", data=kakao_message,
                    file_name=f"{agent_name_display}_{agency_branch}_실적현황.txt", mime="text/plain", use_container_width=True)
                ...
                with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp_file:
                    ...
                    with open(tmp_file.name, "rb") as f:
                        st.download_button(label="📥 리플렛 다운로드", data=f.read(),
                            file_name=f"{agency_name}_leaflet.jpg", mime="image/jpeg", use_container_width=True)
            else:
...
with tab2:
    ...
    with col1:
        ...
        contact_search = st.text_input("검색", placeholder="예: 01012345678, 1234567, 123-4567, 홍길동",
    ...
    if contact_search_clicked:
        if not contact_search:
            ...
            if len(filtered_contacts) == 0:
                ...
                filtered_contacts = filtered_contacts.sort_values('_sort').drop(columns=['_sort']).reset_index(drop=True)
    ...
    if st.session_state.contact_show_duplicates and st.session_state.contact_filtered_data is not None:
        ...
        for idx, (row_idx, contact_row) in enumerate(st.session_state.contact_filtered_data.iterrows()):
            contact_display = f"{str(contact_row.get('지점','N/A')).strip()} | {str(contact_row.get('지사','N/A')).strip()} | {str(contact_row.get('설계사명','N/A')).strip()}"
...
with tab3:
    ...
    if st.session_state.manager_search_performed and st.session_state.manager_agent_list is not None:
        ...
        # ── 인사말 입력 ──
        st.markdown("""
        <div style='background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);padding:8px 14px;border-radius:8px;margin:10px 0 4px 0;'>
        <span style='color:white;font-size:12px;font-weight:500;'>💬 인사말을 입력하면 각 설계사 메시지 맨 앞에 자동으로 추가됩니다.</span>
        </div>""", unsafe_allow_html=True)
        greeting_text = st.text_area("인사말", placeholder="예: 안녕하세요! 이번 주도 파이팅입니다 💪",
            height=70, label_visibility="collapsed", key="manager_greeting")

        # ── 필터 버튼 ──
        filter_labels = ["전체", "현재주차 유실적자", "브릿지 유실적자", "MC 해당자"]
        filter_colors = ["#4a5568", "#3182ce", "#ed64a6", "#805ad5"]
        f_col1, f_col2, f_col3, f_col4 = st.columns(4)
        filter_cols = [f_col1, f_col2, f_col3, f_col4]
        for fi, (fcol, flabel, fcolor) in enumerate(zip(filter_cols, filter_labels, filter_colors)):
            with fcol:
                is_active = st.session_state.manager_filter_mode == fi
                border_style = f"3px solid {fcolor}" if is_active else "2px solid #e2e8f0"
                bg_color = fcolor if is_active else "white"
                txt_color = "white" if is_active else "#4a5568"
                st.markdown(f"""
                <div style='text-align:center;padding:6px 4px;background:{bg_color};
                    border:{border_style};border-radius:8px;cursor:pointer;
                    font-size:12px;font-weight:600;color:{txt_color};
                    box-shadow:0 2px 6px rgba(0,0,0,0.08);margin-bottom:4px;'>
                    {flabel}
                </div>""", unsafe_allow_html=True)
                if st.button(flabel, key=f"filter_btn_{fi}", use_container_width=True):
                    st.session_state.manager_filter_mode = fi
                    st.session_state.manager_expanded_idx = None
                    st.rerun()

        # ── 필터 적용 ──
        filtered_agents = apply_manager_filter(
            all_agents, st.session_state.manager_filter_mode, current_week, df_main
        )

        # ── 헤더 ──
        filter_name = filter_labels[st.session_state.manager_filter_mode]
        st.markdown(f"""
        <div style='background:linear-gradient(135deg,#4a5568 0%,#2d3748 100%);padding:10px 16px;border-radius:10px;margin:8px 0 6px 0;box-shadow:0 2px 8px rgba(74,85,104,0.2);display:flex;justify-content:space-between;align-items:center;'>
        <span style='color:white;font-weight:700;font-size:14px;'>👔 {mgr_name} 매니저</span>
        <span style='color:#ffd93d;font-weight:600;font-size:13px;'>{filter_name} · {len(filtered_agents)}명</span>
        </div>""", unsafe_allow_html=True)

        if len(filtered_agents) == 0:
            st.markdown("""
            <div style='text-align:center;padding:20px;background:white;border-radius:10px;color:#718096;font-size:14px;'>
            해당 조건의 설계사가 없습니다.
            </div>""", unsafe_allow_html=True)
        else:
            # ── 설계사 리스트 ──
            for i, (_, agent_row) in enumerate(filtered_agents.iterrows()):
                ...
                # 순위 색
                if i == 0: rank_bg = "#f6d365"; rank_tc = "#92400e"
                elif i == 1: rank_bg = "#b8c6db"; rank_tc = "#1a365d"
                elif i == 2: rank_bg = "#96e6a1"; rank_tc = "#1c4532"
                else: rank_bg = "#e2e8f0"; rank_tc = "#4a5568"
                ...
                with card_col:
                    st.markdown(f"""
                    <div style='background:white;border-left:4px solid {"#f59e0b" if is_expanded else "#4a5568"};
                        padding:8px 12px;border-radius:8px;margin:2px 0;
                        box-shadow:0 1px 4px rgba(0,0,0,{"0.15" if is_expanded else "0.07"});
                        display:flex;align-items:center;justify-content:space-between;'>
                        <div style='display:flex;align-items:center;gap:8px;'>
                            <div style='background:{rank_bg};color:{rank_tc};border-radius:50%;
                                width:26px;height:26px;display:flex;align-items:center;
                                justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;'>{i+1}</div>
                            <div>
                                <div style='font-size:14px;font-weight:700;color:{"#c05621" if is_expanded else "#2c3e50"};'>{agent_nm}</div>
                                <div style='font-size:11px;color:#718096;'>{agent_office} | {agent_branch}</div>
                            </div>
                        </div>
                        <div style='font-size:14px;font-weight:700;color:#48bb78;'>{cumul_display}</div>
                    </div>""", unsafe_allow_html=True)

                with btn_col:
                    toggle_label = "▲ 접기" if is_expanded else "▼ 상세"
                    if st.button(toggle_label, key=f"toggle_{i}_{agent_nm}", use_container_width=True):
                ...
                if is_expanded:
                    with st.container():
                        st.markdown("<div style='background:#f8f9fa;border-radius:8px;padding:12px;margin:0 0 4px 0;border:1px solid #e2e8f0;'>", unsafe_allow_html=True)

                        is_auth = safe_float(agent_row.get("어센틱구분", 0)) == 1
                        cumulative = agent_row["누계실적"]

                        # 누계실적
                        st.markdown(f"""
                        <div style='font-size:11px;color:#718096;font-weight:600;margin-bottom:2px;'>📈 3월 누계 실적</div>
                        <div style='background:linear-gradient(135deg,#4a5568 0%,#2d3748 100%);padding:8px 12px;border-radius:6px;font-size:16px;font-weight:700;color:white;text-align:center;margin-bottom:8px;'>
                        {format_display(cumulative)}</div>""", unsafe_allow_html=True)

                        # 주차별 실적 (컴팩트)
                        st.markdown("<div style='font-size:11px;color:#718096;font-weight:600;margin-bottom:4px;'>📅 주차별 실적</div>", unsafe_allow_html=True)
                        week_cols_row = st.columns(5)
                        for wi, wc in enumerate(["1주차","2주차","3주차","4주차","5주차"], 1):
                            wv = format_display(agent_row[wc])
                            is_cur = (wi == current_week)
                            bg = "#ffd93d" if is_cur else "white"
                            tc = "#92400e" if is_cur else "#2c3e50"
                            with week_cols_row[wi-1]:
                                st.markdown(f"""
                                <div style='background:{bg};border-radius:6px;padding:5px 4px;text-align:center;
                                    border:1px solid {"#f59e0b" if is_cur else "#e2e8f0"};margin-bottom:4px;'>
                                    <div style='font-size:10px;color:{tc};font-weight:600;'>{wc}{"⭐" if is_cur else ""}</div>
                                    <div style='font-size:11px;color:{"#92400e" if is_cur else "#48bb78"};font-weight:700;'>{wv}</div>
                                </div>""", unsafe_allow_html=True)

                        # 현재주차 목표
                        if is_auth:
                            wt = agent_row.get("어센틱주차목표","0"); ws = agent_row.get("어센틱주차부족","0")
                        else:
                            wt = agent_row.get("주차목표","0"); ws = agent_row.get("주차부족최종","0")
                        st.markdown(f"""
                        <div style='background:white;border-left:3px solid #ed8936;padding:6px 10px;border-radius:6px;margin:4px 0;font-size:12px;'>
                        <strong>⭐ 주차목표:</strong> {format_display(wt)} &nbsp;|&nbsp; <strong>부족:</strong> {format_display(ws)}
                        </div>""", unsafe_allow_html=True)
                        ...
                        if is_auth:
                            ...
                            if mc_tv == 0 or mc_tv == 2:
                                mc_status = "이번달 20만원 도전"
                                mc_color = "#ed8936"
                            elif any(x in mc_s for x in ["최종달성"]) or safe_float(mc_s) <= 0:
                                mc_status = "✅ 시상금확보"
                                mc_color = "#48bb78"
                            else:
                                mc_status = "🟡 도전중"
                                mc_color = "#ed8936"
                            st.markdown(f"""
                            <div style='background:white;border-left:3px solid #fc8181;padding:6px 10px;border-radius:6px;margin:4px 0;font-size:12px;'>
                            ...
                            st.markdown(f"""
                            <div style='background:white;border-left:3px solid #ed64a6;padding:6px 10px;border-radius:6px;margin:4px 0;font-size:12px;'>
                        ...
                        if mp_val <= 0 or "최종달성" in mp_s:
                            mp_status = "✅ 시상금확보"; mp_color = "#48bb78"
                        elif any(x in mp_s for x in ["대상아님","미달성","다음기회에"]):
                            mp_status = "⚪ 대상아님"; mp_color = "#718096"
                        else:
                            mp_status = "🟡 도전중"; mp_color = "#805ad5"
                        st.markdown(f"""
                        <div style='background:white;border-left:3px solid #805ad5;padding:6px 10px;border-radius:6px;margin:4px 0;font-size:12px;'>
                        ...
                        copy_to_clipboard_button(msg, button_label=f"📋 {agent_nm} 메시지 복사", key=f"mgr_copy_{i}_{agent_nm}", height=80)
        ...
        <p style='color:#48bb78;font-weight:500;font-size:12px;margin-top:12px;'>✨ 필터로 원하는 대상만 선택하여 메시지를 발송하세요!</p>
        </div>""", unsafe_allow_html=True)
