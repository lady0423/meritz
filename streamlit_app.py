...
def copy_to_clipboard_button(text, button_label="📋 메시지 복사하기", key="clipboard_btn"):
    """JavaScript를 이용한 클립보드 복사 버튼"""
    # JS에서 안전하게 쓰기 위해 특수문자 이스케이프
...
이번주도 화이팅입니다!"""
            ...
            # ✅ 수정1 적용: 복사 버튼 + 다운로드 버튼
            col_copy1, col_copy2 = st.columns([1, 1])
            
            with col_copy1:
                # JavaScript 클립보드 복사 버튼
    ...
    if contact_search_clicked:
        if not contact_search:
            ...
            if len(filtered_contacts) == 0:
                ...
                # ✅ 수정2: 지점 컬럼 기준 오름차순 정렬
                filtered_contacts = filtered_contacts.sort_values(by="지점", ascending=True).reset_index(drop=True)
