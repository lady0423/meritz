    ...
    if not manager_name or not agent_code:
        ...
        try:
            ...
            if filtered.empty:
                ...
                # 주차별 값
                weekly_values = [
                    safe_float(safe_get_value(row, "1주차")),    # M열
                    safe_float(safe_get_value(row, "2주차")),    # N열
                    safe_float(safe_get_value(row, "3주차")),    # O열
                    safe_float(safe_get_value(row, "4주차")),    # P열
                    safe_float(safe_get_value(row, "5주차")),    # Q열
                ]
                
                # 브릿지
                bridge_progress = safe_float(safe_get_value(row, "브릿지 실적"))    # H열
                bridge_target = safe_float(safe_get_value(row, "브릿지 도전구간"))    # I열
                bridge_shortage = safe_float(safe_get_value(row, "브릿지 부족"))      # J열
                
                # MC+
                mc_challenge = safe_float(safe_get_value(row, "MC+구간"))    # T열
